...
if [ -n "$DEVBOX_SHELL" ]; then
    ...
    echo "[1/2] AST Analysis - extracting gen8 code paths..."
    go run ./cmd/ast-analyzer --mesa "$MESA_SRC" --out "$OUTDIR/analysis.json"
    echo ""

    echo "[2/2] Generating Graphviz DOT diagrams..."
    go run ./cmd/graph-gen --input "$OUTDIR/analysis.json" --outdir "$OUTDIR/"
    echo ""

    echo "[render] Rendering DOT → SVG (parallel sfdp)..."
    for f in "$OUTDIR"/*.dot; do
        base="$(basename "$f" .dot)"
        (sfdp -Tsvg -Goverlap=false -Gsplines=true "$f" -o "$OUTDIR/$base.svg" 2>/dev/null && echo "  ✓ $base.svg") &
    done
    wait
    echo ""
    ...
    echo ""
    echo "Open: xdg-open $OUTDIR/01_turnip_gen8_architecture.svg"
    echo "      xdg-open $OUTDIR/02_gen8_neuron_paths.svg"
    echo "      xdg-open $OUTDIR/03_call_graph_focused.svg"
    ...
    echo "Or run manually: cd $DIR && go run ./cmd/..."
    MESA_SRC="$MESA_SRC" OUTDIR="$OUTDIR" bash "$0" "$@" 2>/dev/null || true
fi
